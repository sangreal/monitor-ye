...
import enn.monitor.framework.ai.nn.weights.NNWeightFactory;
...
public class NNActivationSigmoid implements NNActivationInterface {
	...
	public void backPropagate(NNObject nnObject, NNWeightEnum nnWeightEnum,
				...
				for (i = 0; i < weightListList.size(); ++i) {   // last
					...
					for (j = 0; j < weightListList.get(0).size() - 1; ++j) {   // front
						nnRelation.getOffWeightListList().get(i).set(
								j, nnRelation.getErrorList().get(i) * nnObject.getNNData(k).getyList().get(j));
					}
					nnRelation.getOffWeightListList().get(i).set(
							j, nnRelation.getErrorList().get(i) * nnObject.getNNData(k).getBias());
				}
			} else {    // 隐藏层
				weightListList = nnRelation.getWeightListList();
				for (i = 0; i < weightListList.size(); ++i) {   // last
		...
		NNWeightFactory.flushWeight(nnObject, nnWeightEnum);
	}
}
