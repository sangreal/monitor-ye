...
public class NNObject {
	...
	public NNObject cloneNNObject() {
		...
		nnOjbect.setMomentum(momentum);
		nnOjbect.setLearningRate(learningRate);
		nnOjbect.setError(error);
		nnOjbect.setMaxNoise(maxNoise);
	...
	public void adjustBias(double min, double max) {
		int i;
		NNLayer nnLayer = null;
		
		for (i = 0; i < numNeuronsList.size(); ++i) {
			nnLayer = numNeuronsList.get(i);
			nnLayer.setBias(NNParameterAdjustUtil.adjustParameter(nnLayer.getBias(), min, max));
			logger.debug("Bias: " + nnLayer.getBias());
		}
	}
...
//		private NNLayer() {}
		...
		public NNLayer cloneNNData() {
			NNLayer nnData = new NNLayer(numNeurons, bias);
	...
	public static class NNRelation {
		...
		public void addOffWeightList(List<Double> offWeightList) {
			offWeightListList.add(offWeightList);
		}
		...
		public void addPreOffWeightList(List<Double> preOffWeightList) {
			preOffWeightListList.add(preOffWeightList);
		}
...
//		}
		...
		public NNRelation cloneNNRelation() {
			...
			for (i = 0; i < offWeightListList.size(); ++i) {
				nnRelation.addOffWeightList(new ArrayList<Double>(offWeightListList.get(i)));
			}
			for (i = 0; i < preOffWeightListList.size(); ++i) {
				nnRelation.addPreOffWeightList(new ArrayList<Double>(preOffWeightListList.get(i)));
			}
			nnRelation.getErrorList().addAll(errorList);
