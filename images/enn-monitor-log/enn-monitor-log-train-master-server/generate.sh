...
filePath=$1/enn-monitor-log-train-master-server-rc.yml
cp enn-monitor-log-train-master-server-rc.yml $filePath
...
filePath=$1/enn-monitor-log-train-master-server-svc.yml
cp enn-monitor-log-train-master-server-svc.yml $filePath
sed -i -e "s/%NAMESPACE%/$1/g" $filePath
sed -i -e "s/%LISTENPORT%/$LISTENPORT/g" $filePath
sed -i -e "s/%EXTERNPORT%/$EXTERNPORT/g" $filePath

ennctl -n $1 create configmap log-train-master-conf --from-file config/monitor.properties --from-file $1/run.sh
