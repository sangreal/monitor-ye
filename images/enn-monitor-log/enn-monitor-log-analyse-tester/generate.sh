...
filePath=$1/enn-monitor-log-analyse-tester-rc.yml
cp enn-monitor-log-analyse-tester-rc.yml $filePath
...
filePath=$1/enn-monitor-log-analyse-tester-svc.yml
cp enn-monitor-log-analyse-tester-svc.yml $filePath
sed -i -e "s/%NAMESPACE%/$1/g" $filePath
sed -i -e "s/%LISTENPORT%/$LISTENPORT/g" $filePath
sed -i -e "s/%EXTERNPORT%/$EXTERNPORT/g" $filePath

ennctl -n $1 create configmap log-analyse-tester-conf --from-file config/monitor.properties --from-file $1/run.sh
