...
filePath=$1/enn-monitor-log-config-gateway-server-rc.yml
cp enn-monitor-log-config-gateway-server-rc.yml $filePath
...
filePath=$1/enn-monitor-log-config-gateway-server-svc.yml
cp enn-monitor-log-config-gateway-server-svc.yml $filePath
sed -i -e "s/%NAMESPACE%/$1/g" $filePath
sed -i -e "s/%LISTENPORT%/$LISTENPORT/g" $filePath
sed -i -e "s/%EXTERNPORT%/$EXTERNPORT/g" $filePath

kubectl -n $1 create configmap log-config-gateway-conf --from-file config/monitor.properties --from-file $1/run.sh
