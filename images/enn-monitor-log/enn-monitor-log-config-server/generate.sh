...
filePath=$1/enn-monitor-log-config-server-rc.yml
cp enn-monitor-log-config-server-rc.yml $filePath
...
filePath=$1/enn-monitor-log-config-server-svc.yml
cp enn-monitor-log-config-server-svc.yml $filePath
sed -i -e "s/%NAMESPACE%/$1/g" $filePath
sed -i -e "s/%LISTENPORT%/$LISTENPORT/g" $filePath
sed -i -e "s/%EXTERNPORT%/$EXTERNPORT/g" $filePath

kubectl -n $1 create configmap log-config-conf --from-file config/monitor.properties --from-file $1/run.sh
