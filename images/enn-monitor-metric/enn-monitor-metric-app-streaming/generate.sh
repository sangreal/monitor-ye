...
# enn-monitor-metric-app-streaming
filePath=$1/enn-monitor-metric-app-streaming-rc.yml
cp enn-monitor-metric-app-streaming-rc.yml $filePath
...
filePath=$1/enn-monitor-metric-app-streaming-svc.yml
cp enn-monitor-metric-app-streaming-svc.yml $filePath
sed -i -e "s/%NAMESPACE%/$1/g" $filePath
sed -i -e "s/%LISTENPORT%/$LISTENPORT/g" $filePath
sed -i -e "s/%EXTERNPORT%/$EXTERNPORT/g" $filePath

kubectl --namespace=$1 create configmap metric-streaming-conf --from-file config/start.sh
