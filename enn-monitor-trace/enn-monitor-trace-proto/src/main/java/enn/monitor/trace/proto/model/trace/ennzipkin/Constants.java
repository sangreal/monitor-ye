...
public final class Constants {
     ...
     *
     * Unlike RPC annotations like {link #CLIENT_SEND}, messaging spans never share a span ID. For
     * example, "ms" should always be the parent of "mr".
     *
     * {link Annotation#endpoint} is not the destination, it is the host which logged the send
     ...
     *
     * Unlike RPC annotations like {link #SERVER_RECV}, messaging spans never share a span ID. For
     * example, "mr" should always be a child of "ms" unless it is a root span.
     *
     * {link Annotation#endpoint} is not the broker, it is the host which logged the receive
     ...
     *
     * Due to transient errors, an ERROR annotation should not be interpreted as a span failure,
     ...
     *
     * Here's an example: A span has an ERROR annotation, added when a WIRE_SEND failed. Another
     * WIRE_SEND succeeded, so there's no ERROR binary annotation on the span because the overall
     * operation succeeded.
     *
     * Note that RPC spans often include both client and server hosts: It is possible that only one
