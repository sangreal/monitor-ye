...
import static enn.monitor.trace.proto.model.trace.ennzipkin.internal.Util.assertionError;
import static enn.monitor.trace.proto.model.trace.ennzipkin.internal.Util.checkArgument;
...
//    }
        ...
        @Override public Span read(ByteBuffer bytes) {
            ...
            while (true) {
                ...
                if (field.isEqualTo(TRACE_ID)) {
                    builder.traceId(readUtf8(bytes));
                } else if (field.isEqualTo(NAME)) {
                    builder.resource(readUtf8(bytes));
                } else if (field.isEqualTo(ID)) {
                    builder.id(readUtf8(bytes));
                } else if (field.isEqualTo(PARENT_ID)) {
                    builder.parentId(readUtf8(bytes));
    ...
    static final Buffer.Writer<Span> SPAN_WRITER = new Buffer.Writer<Span>() {
        @Override public int sizeInBytes(Span value) {
            ...
            sizeInBytes += 3 + 4 + Buffer.utf8SizeInBytes(value.resource);
            sizeInBytes += 3 + 8;// ID
        ...
        @Override public void write(Span value, Buffer buffer) {
            ...
            buffer.writeAscii(value.getTraceId());

            SpanReader.NAME.write(buffer);
            buffer.writeLengthPrefixed(checkStringLength(value.resource, STRING_LENGTH_LIMIT));

            SpanReader.ID.write(buffer);
            buffer.writeAscii(value.getId());

            if (value.parentId != null) {
                SpanReader.PARENT_ID.write(buffer);
                buffer.writeAscii(value.getParentId());
            }
